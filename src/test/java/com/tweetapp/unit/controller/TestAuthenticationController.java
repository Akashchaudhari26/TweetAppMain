...
public class TestAuthenticationController {
	...
	public void testRegisterUser_ValidInput() throws Exception {
		...
		when(authenticationService.registerNewUser(isA(UserRegisterRequest.class))).thenReturn(User.buildUser(userRegisterRequest));
		
		String valueAsString = objectMapper.writeValueAsString(userRegisterRequest);

		MvcResult mvcResult = mockMvc.perform(
				post("/api/v1.0/tweets/register").content(valueAsString)
	...
	public void testRegisterUser_InvalidFirstName() throws Exception {
		...
		when(authenticationService.registerNewUser(isA(UserRegisterRequest.class))).thenThrow(InvalidOperationException.class);
	...
	public void testPasswordChange_Valid() throws Exception {
		ForgotPasswordRequest forgotPasswordRequest = ForgotPasswordRequest.builder().loginId("test_user").password("Update@Password2").build();
		...
		mockMvc.perform(
				get("/api/v1.0/tweets/test_user/forgot").content(valueAsString)
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void testPasswordChange_InValidPasswordFormat() throws Exception {
		ForgotPasswordRequest forgotPasswordRequest = ForgotPasswordRequest.builder().loginId("test_user").password("Updateassw").build();
		
		String valueAsString = objectMapper.writeValueAsString(forgotPasswordRequest);
		
		when(authenticationService.changePassword(isA(ForgotPasswordRequest.class))).thenThrow(InvalidOperationException.class);
